...
Converter.convert = function(baseURL, conf) {
  ...
  resources.forEach(function(res) {
    ...
    var instOps = (swagger.paths["/" + res.type + "/{id}"] = {
      parameters: [{ in: "path", required: true, name: "id", type: "string" }]
    });
    ...
    if (interactions.indexOf("search-type") !== -1) {
      ...
      typeOps.get.responses["200"].schema = {
        type: "array",
        items: { $ref: schemaRef }
      };
    }
    if (interactions.indexOf("history-instance") !== -1) {
      var histOp = (swagger.paths["/" + res.type + "/{id}/_history"] = {
    ...
    if (interactions.indexOf("history-type") !== -1) {
      var histOp = (swagger.paths["/" + res.type + "/_history"] = {
        get: getDefaultOp(res)
      });
      histOp = histOp.get;
      histOp.parameters = [
    ...
    if (interactions.indexOf("vread") !== -1) {
      var versionOp = (swagger.paths[
        "/" + res.type + "/{id}/_history/{vid}"
      ] = { get: getDefaultOp(res) });
